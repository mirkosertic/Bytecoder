...
public class CPPSSAWriter {
    ...
    protected final CPPPrintWriter writer;
    ...
    public CPPSSAWriter withDeeperIndent() {
        return new CPPSSAWriter(options, program, indent + 1, writer, linkerContext, constantPool, labelRequired, minifier, allocator, idResolver, analysisStack);
    }

    public CPPPrintWriter startLine() {
    ...
    public void printRegisterDeclarations() {
        ...
        for (final Register r : theList) {
            final CPPPrintWriter thePW = startLine().text(minifier.typeRefToString(r.getType())).text(" ").text(toRegisterName(r)).text(";");
    ...
    private void print(final ResolveCallsiteInstanceExpression aValue) {
        ...
        final CPPSSAWriter theNested = new CPPSSAWriter(options, program, indent + 1, writer, linkerContext, constantPool, labelRequired, minifier, theAllocator, idResolver, analysisStack);
...
//                writer.assignPositionToSourceFile(thePosition);
    ...
    private void writeExpression(final Expression aExpression) {
        ...
        if (aExpression instanceof ReturnExpression) {
            ...
            final CPPPrintWriter theWriter = startLine();
            ...
            final CPPSSAWriter theDeeper = withDeeperIndent();
    ...
    private void print(final Relooper.TryBlock aTryBlock) {
        ...
        final CPPSSAWriter theHandler = withDeeperIndent();

        final Relooper.Block theFinally = aTryBlock.getFinallyBlock();
        CPPSSAWriter theGuard = theHandler;
        if (theFinally != null) {
        ...
        if (theFinally != null) {
            ...
            final CPPSSAWriter theFinallyDeeper = theHandler.withDeeperIndent();
    ...
    public void printStackified(final Stackifier stackifier) {
        final ArrayDeque<CPPSSAWriter> writerStack = new ArrayDeque<>();
        ...
        final Stackifier.StackifierStructuredControlFlowWriter writer = new Stackifier.StackifierStructuredControlFlowWriter(stackifier) {
            ...
            public void beginLoopFor(final Block<RegionNode> block) {
                ...
                final CPPSSAWriter current = writerStack.peek();
                current.startLine().label(block.getLabel())
                        .text(":").space()
                        .text("for(;;)").space().text("{").newLine();
                final CPPSSAWriter newLoopBlock = current.withDeeperIndent();
            ...
            public void beginBlockFor(final Block<RegionNode> block) {
                ...
                final CPPSSAWriter newSimpleBlock = current.withDeeperIndent();
