...
public class InefficientFieldWriteOptimizer extends RecursiveExpressionVisitor implements Optimizer {
    ...
    protected void visit(final ControlFlowGraph aGraph, final ExpressionList aList, final Expression aExpression, final BytecodeLinkerContext aLinkerContext) {
        if (aExpression instanceof PutFieldExpression) {
            final PutFieldExpression thePutField = (PutFieldExpression) aExpression;
            final List<Value> theIncoming = thePutField.incomingDataFlows();
            boolean repeat = true;
            while(repeat) {
                repeat = false;
                ...
                if (theBefore != null) {
                    ...
                    if (theBefore instanceof VariableAssignmentExpression) {
                        ...
                        if (theDataEdges.size() == 1 && theVariable == theValue && !theVariable.getName().startsWith("local_")) {
                            thePutField.replaceIncomingDataEdge(theValue, theAssignment.getValue());
                        ...
                        if (theDataEdges.size() == 1 && theVariable == theTarget && !theVariable.getName().startsWith("local_")) {
                            thePutField.replaceIncomingDataEdge(theTarget, theAssignment.getValue());
                            aList.remove(theAssignment);
                            aGraph.getProgram().deleteVariable(theVariable);
                            repeat = true;
                        }
        ...
        if (aExpression instanceof PutStaticExpression) {
            final PutStaticExpression thePutField = (PutStaticExpression) aExpression;
            final List<Value> theIncoming = thePutField.incomingDataFlows();
            boolean repeat = true;
            while(repeat) {
                repeat = false;
                final Expression theBefore = aList.predecessorOf(aExpression);
                if (theBefore != null) {
                    final Value theValue = theIncoming.get(0);
