...
public class ControlFlow {
    ...
    ControlFlow(final AbstractInsnNode currentNode, final List<AbstractInsnNode> visitedLabels, final GraphParserState graphParserState) {
        this.currentNode = currentNode;
        this.visitedLabels = visitedLabels;
        this.graphParserState = graphParserState;
    }
    public boolean visited(final AbstractInsnNode node) {
        return visitedLabels.contains(node);
    }

    public ControlFlow addInstructionAndContinueWith(final AbstractInsnNode instruction, final AbstractInsnNode next) {
        final List<AbstractInsnNode> newVisited = new ArrayList<>(visitedLabels);
        newVisited.add(instruction);
