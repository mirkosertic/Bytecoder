    ...
    private Device findBestDevice(final List<Platform> platforms, final OpenCLOptions clOptions) {
        ...
        return platforms.stream()
                ...
                .flatMap(p -> p.deviceList
                        .stream()
                        .filter(d -> preferredDeviceIndex.map(index -> index == d.index).orElse(true))).min((a, b) -> clOptions.getPreferredDeviceComparator().compare(b.deviceProperties, a.deviceProperties))
    ...
    private Optional<Integer> getSystemPropertyAsInt(final String name) {
        ...
        if (propertyLiteral != null && propertyLiteral.length() > 0) {
            try {
                ...
                logger.warn("cannot parse system property '{}' to an integer value '{}'", name);
            }
        }
