...
public abstract class UnicodeEncoder extends CharsetEncoder {
    ...
    private final int byteOrder;      /* Byte order in use */
    private final boolean usesMark;   /* Write an initial BOM */
    private boolean needsMark;

    protected UnicodeEncoder(final Charset cs, final int bo, final boolean m) {
        super(cs, 2.0f,
    ...
    private void put(final char c, final ByteBuffer dst) {
        if (byteOrder == BIG) {
            dst.put((byte)(c >> 8));
            dst.put((byte)(c & 0xff));
        } else {
